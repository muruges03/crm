...
<template>
    ...
    <div class="lg:pl-64 flex flex-col bg-white divide-x divide-gray-200 border-2 shadow-md ">
        ...
        <div class="flex flex-col p-4">
            <div class="border-b border-gray-200 px-2 py-2 flex sm:items-center justify-between sm:px-4 lg:px-8">
                ...
                <div class=" sm:mt-0 sm:ml-4 " >
                    <jet-button type="button" >
                        <inertia-link v-bind:href="route('payment.create')">Create</inertia-link>
                    </jet-button>
                </div>
            </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-3 py-3 text-md text-center  ">
                                    <inertia-link>Type</inertia-link>
                                </th>
                                <th scope="col" class="px-3 py-3 text-md text-center  ">
                                    <inertia-link>Patron Name</inertia-link>
                                </th>
                                <th scope="col" class="px-3 py-3 text-md text-center  ">
                                    <inertia-link>Date</inertia-link>
                                </th>
                                <th scope="col" class="px-3 py-3 text-md text-center  ">
                                    <inertia-link>Amount</inertia-link>
                                </th>
                                <th class="px-3 py-3 text-md text-center  " ></th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-for="(payments, index) in completed.data" :key="payments.id" v-bind:class="index % 2 === 0 ? 'bg-white-100 hover:bg-blue-100' : 'tr-bg-color hover:bg-blue-100'">
                                <td  class="px-3 py-3 text-sm text-center text-gray-900">
                                    <inertia-link  >{{ index+1 }}</inertia-link>
                                </td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ payments.type }}</inertia-link></td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ payments.patron_id }}</inertia-link></td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ payments.payment_date }}</inertia-link></td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ payments.total_amount }}</inertia-link></td>
                                <td class="py-4 pl-3 pr-4 flex justify-center text-sm font-medium sm:pr-6">

                                    <inertia-link class="" v-bind:href="route('payment.edit', payments.id)" >
                                    ...
                                    <a  class="btn btn-danger btn-sm" @click="destroy(payments.id)" >
            ...
            <pagination class="" :links="completed.links" />
        </div>
    </div>
</template>
        ...
        destroy(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(this.route('payment.destroy', id))
                }
            })
        },
        search() {
            this.$inertia.replace(this.route('payment.index', {term: this.term}))
        },
        resetPagination(length){
            //console.log(length);
            this.$inertia.replace(this.route('ledger', {length: this.length}))
        },
    },
})
</script>
