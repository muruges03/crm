            ...
            <form class="bg-white-300 shadow-lg" @submit.prevent="store">
                ...
                <div class="grid grid-cols-12 gap-2 px-4">
                    <div class="col-span-12 sm:col-span-3">
                        ...
                        <div class="relative">
                            <Multiselect   :multiple="false" v-model="form.payment_type" :class="`${form.errors.payment_type ? 'border-red-500 placeholder-text-300 placeholder-red-300' : ''}`"  select-label="" deselect-label="" class="!block"
                    ...
                    <div class="col-span-12 sm:col-span-3">
                        <label for="description" class="mixlabel ">Paid Status</label>
                        <Switch  v-model="form.verified_bank"  :class="[form.verified_bank ? 'bg-green-500' : 'bg-gray-200', 'mt-1 relative inline-flex flex-shrink-0  h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-300']">
                            <span class="sr-only">Use setting</span>
                            <span :class="[form.verified_bank ? 'translate-x-full' : 'translate-x-0', 'pointer-events-none relative inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200']">
                                <span :class="[form.verified_bank ? 'opacity-0 ease-out duration-100' : 'opacity-100 ease-in duration-200', 'absolute inset-0 h-full w-full flex items-center justify-center transition-opacity']" aria-hidden="true">
                                    <svg class="h-3 w-3 text-gray-400" fill="none" viewBox="0 0 12 12">
                                    <path d="M4 8l2-2m0 0l2-2M6 6L4 4m2 2l2 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </span>
                                <span :class="[form.verified_bank ? 'opacity-100 ease-in duration-200' : 'opacity-0 ease-out duration-100', 'absolute inset-0 h-full w-full flex items-center justify-center transition-opacity']" aria-hidden="true">
...
<!--                        <jet-button type="button" @click="destory()">-->
<!--                            Delete-->
<!--                        </jet-button>-->
...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                payment_type        : this.paymenttransaction.payment_type,
                vendor_id           : this.paymenttransaction.vendor_id,
                payment_date        : this.paymenttransaction.payment_date,
                ledger_id           : this.paymenttransaction.ledger_id,
                description         : this.paymenttransaction.description,
                total_amount        : this.paymenttransaction.total_amount,
                paid_amount         : this.paymenttransaction.total_amount,
                journal_status      : this.paymenttransaction.journal_status,
                verified_bank       : this.paymenttransaction.verified_bank,
                currentPage         : this.paymenttransaction.currentPage,
                bill_ref_id         : this.paymenttransaction.bill_ref_id,
    ...
    methods: {
        store() {
            // alert();
            this.form.put(this.route('payment.update',this.paymenttransaction.id))
        },
    },
}
</script>
