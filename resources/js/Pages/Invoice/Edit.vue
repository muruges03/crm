    ...
    <div class="lg:pl-64">
        <div class=" md:px-4 bg-white md:py-6 sm:p-3">
            <div class="flex px-4">
                ...
                <inertia-link :href="route('invoice')" >UPDATE INVOICE</inertia-link>
            ...
            <div class="shadow overflow-visible sm:rounded">
                    ...
                    <div class="grid grid-cols-12 gap-2 md:px-2">
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            ...
                            <div class="mt-1 sm:mt-0 sm:col-span-2">
                                <div class="max-w-lg flex rounded ">
                                    <jet-input  v-model="form.prefix" type="text" disabled style="width:70px;" required name="prefix" id="prefix"  class="inline-flex rounded-l-md border border-r-0" />
                                    <jet-input  v-model="form.invoice_number" disabled ref="invoice_number" type="number"  name="invoice_number" required id="invoice_number"   class="flex-1  rounded-none rounded-r-md " />
                                </div>
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            ...
                            <div class="sm:col-span-2">
                                <Datepicker class="w-full" ref="invoiced_at"  name="invoiced_at" v-model="form.invoiced_at" autoApply :enableTimePicker="false"
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            ...
                            <div class="sm:col-span-2">
                                <Datepicker class="w-full" ref="due_at"  name="due_at" v-model="form.due_at" autoApply :enableTimePicker="false"
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            ...
                            <div class="relative">
                                <Multiselect :vlaue="this.ledger.id" :multiple="false"  :group-select="false" v-model="form.ledger_id" class="!block"
                        ...
                        <div class="col-span-12 sm:col-span-3">
                            <label for="paid_date" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2 pt-5"> Paid Date </label>
                            <div class="sm:col-span-2 ">
                                <Datepicker class="block max-w-lg w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md"
                                     ref="paid_date" name="paid_date" v-model="form.paid_date" autoApply :enableTimePicker="false"
                                    id="paid_date"  autocomplete="paid_date" placeholder="Start Date" />
                            </div>
                            <div v-if="errors.paid_date" class="text-xs text-red-500">The Paid Date is required.</div>
                        </div>
                        <div class="col-span-12 sm:col-span-3">
                            <label for="paid_status" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2 pt-5"> Paid Status </label>
                            <Switch  @click="paidstatus(form)" :class="[form.paid_status ? 'bg-green-500' : 'bg-gray-200', 'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500']">
                        ...
                        <!--<div  class="col-span-12 sm:col-span-3">
                            <label for="start_date" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2 pt-5"> Email </label>
                            <div class="sm:col-span-2">
                                <input  v-model="form.email" ref="email" type="email" name="email" id="email" autocomplete="email"
                                 class="max-w-lg block w-full  focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded">
                            </div>
                            <div v-if="errors.email" class="text-xs text-red-500">The Email is required.</div>
                        </div>-->
                    </div>
                </div>
                <div class="px-6 ">
                    <div class=" overflow-visible ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded">
                        <table class="min-w-full ">
                            <tbody class="">
                                <tr v-for="(add, index) in invoiceiteam" :key='add'>
                                    <div class="bg-[#4989a8] border text-white flex justify-between px-2 py-2 rounded text-base"><span>Row : {{index+1}} </span>
                                         <svg xmlns="http://www.w3.org/2000/svg" @click="deleteRow(index, add,invoiceiteam)" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" >
                                    ...
                                    <div class="grid grid-cols-12  gap-2 py-1">
                                        ...
                                        <div class="col-span-12 sm:col-span-3">
                                            <!-- <infra-label for="title" class="mixlabel">Tax</infra-label> -->
                                        ...
                                        <div class="col-span-12 sm:col-span-2">
                                            ...
                                            <div class="relative rounded shadow-sm">
                                                ...
                                                <jet-input type="number" v-model="add.it_totamt" ref="total_amount" name="total_amount" id="total_amount" style="padding-left:17px !important"  placeholder="Total Amount " autocomplete="total_amount" class="" />
                                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                    <span class="text-gray-500 sm:text-sm" id="price-currency"> INR </span>
                                                </div>
                                            </div>
                ...
                <div class="md:grid md:grid-cols-3 md:gap-2 md:px-6">
                         ...
                         <div class="px-4  bg-white sm:pr-2">
                            ...
                            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:pt-1 flex justify-between py-1">
                                <label for="discount_amount" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2 sm:col-span-2"> Discount (₹) </label>
                                 <label for="discount_amount"  class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2" style="text-align-last: end;"> {{formatNumber(form.i_disamt)}}</label>
                            </div>
                            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:pt-1 flex justify-between py-1">
                                <label for="amount_tax" class="sm:col-span-2 block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"> Tax </label>
                                <label for="amount_tax" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2 items-end" style="text-align-last: end;"> {{formatNumber(form.i_taxamt)}} </label>
                            </div>
                            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:pt-1 flex justify-between py-1">
                                ...
                                <div class="relative rounded shadow-sm flex justify-end">
                                    ...
                                    <label  class="block text-sm font-medium text-gray-700 sm:mt-px items-end" style="text-align-last: end;">{{formatNumber(form.i_totamt)}}</label>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="px-4 py-3 bg-gray-50 flex text-right sm:px-6 justify-end">
                    <div class="flex " v-if="this.invoice[0]['paid_status']==0">
                        <button type="button"  @click='update(form,invoiceiteam)' class="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" >Update</button>
                    </div>
                   <!-- <div class="flex ">
                        <button type="button"  @click='mail(form)' class="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm
                        font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Send Mail</button>
                    </div>-->
                </div>
            </div>
        </form>
...
<script>
    ...
    export default ({
        ...
        AppLayout,
        AppMenu,JetInput,
        Welcome,Multiselect,moment,Swal,Datepicker,Switch
        },
        setup(props) {
            const form = useForm({
                    id              : props.invoice[0].i_id,
                    customer_name   : props.patronid[0],
                    prefix          : props.invoice[0].prefix,
                    invoice_number  : props.invoice[0].invoice_number,
                    order_number    : props.invoice[0].order_number,
                    status          : props.invoice[0].status,
                    start_date      : props.invoice[0].start_date,
                    end_date        : props.invoice[0].end_date,
                    invoiced_at     : props.invoice[0].invoiced_at,
                    due_at          : props.invoice[0].due_at,
                    ledger_id       : props.ledgerid[0],
                    untaxed_amount  : props.invoice[0].untaxed_amount,
                    i_taxamt        : props.invoice[0].i_taxamt,
                    i_totamt        : props.invoice[0].i_totamt,
                    i_disamt        : props.invoice[0].i_disamt,
                    notes           : props.invoice[0].notes,
                    type            : props.invoice[0].type,
                    is_sent         : props.invoice[0].is_sent,
                    //email           : '',
                    paid_status     : props.invoice[0].paid_status,
                    paid_date       : null,

            });
                return {form};
        },
        props:['invoice','invoiceiteam','tax','product','patron','patronid','invoicedata','email','paiddate','ledger','ledgerid'],
        // ,'ledger'
        data() {
            return {
                yearly: false,
    ...
    methods: {
        ...
        deleteRow(index, add,invoiceiteam) {
            if(invoiceiteam.length > 1){
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    // axios.get(this.route('invoice.rowdelete',add))
                    //     .then(response => {
                    //         this.rowdelete = response.data
                    //     })
                    //     .catch(error => {
                    //         console.log(error);
                    // })
                    if(add.tax_id!='') add.tax_id = add.tax_id.id;
                        else add.tax_id = '';
                    this.itemdelete.push({id:add.id,tax_id:add.tax_id});
                    var idx = this.invoiceiteam.indexOf(add);

                    if (idx > -1) {
                        this.invoiceiteam.splice(idx, 1);
                    }
                    this.calculateTotal();
                }
                })
        ...
        calculateTotal() {
            ...
            total = this.invoiceiteam.reduce(function (sum, product) {
                var lineTotal = parseFloat(product.it_totamt);
            ...
            // console.log( subtotal,totalamount,totaltaxamount,totaldiscount);
            ...
            if (!isNaN(totalamount)){
                ...
                this.form.i_totamt = totalamt.toFixed(2);
            }else{  this.form.i_totamt = '0.00';}
            if (!isNaN(totaltaxamount)) this.form.i_taxamt = totaltaxamount.toFixed(2);
                        else  this.form.i_taxamt = '0.00';
            if (!isNaN(totaldiscount)) this.form.i_disamt = totaldiscount.toFixed(2);
                        else  this.form.i_disamt = '0.00';
            // console.log(subtotal,total);
        },
        calculateLineTotal(add) {
            // console.log('calculateLineTotal = ',add);
            var total = parseFloat(add.price) * parseFloat(add.quantity);
            var taxtotal= (total * (add['tax_id']['tax_amount']/100));
            if (!isNaN(total))  add.it_totamt = total.toFixed(2);
                else add.it_totamt = '0';
                if (!isNaN(taxtotal))  add.it_taxamt = taxtotal.toFixed(2);
                else  add.it_taxamt = '0';
            if(add.discount_type=="%") add.total_discount  = (total * (add.it_disamt/100))
                else add.total_discount  = add.it_disamt;
                // console.log('calculateLineTotal = ',total,add,add.it_disamt,add.total_discount);
            this.calculateTotal();
        },
        invoiceType(event){
        ...
        taxremove(add){
            ...
            add.tax_id={id: 0, tax_name: '', tax_type: '', tax_amount: '0',};
            add.it_taxamt = 0;

            this.calculateTotal();
        },
        date(){
            if(this.form.type=='Monthly'){ this.monthly=true; this.yearly=false;}
                else{ this.yearly=true; this.monthly=false;}
            if(this.form.invoiced_at == null) this.form.invoiced_at=null;
                else this.form.invoiced_at=moment(this.form.invoiced_at).format('YYYY-MM-DD');
            if(this.form.due_at == null) this.form.due_at=null;
                else this.form.due_at=moment(this.form.due_at).format('YYYY-MM-DD');
            if(this.paiddate == null) this.form.paid_date=null;
                else this.form.paid_date=moment(this.paid_date).format('YYYY-MM-DD');
            // console.log(this.form.invoiced_at);
        },
        update(form,invoiceiteam){
            if(form.customer_name==null)  return [this.errors.customer_name=true];
            else this.errors.customer_name=false;
            if(form.invoice_number==null) return [this.errors.invoice_number=true]
            else this.$refs.invoice_number.focus();
            ...
            for (var i = 0; i < invoiceiteam.length; i++){
                // console.log(addrows[i].product_id);
                if(invoiceiteam[i].product_id=='' || invoiceiteam[i].product_id==null)  return this.adderrors.product_id=true;
                    else this.adderrors.product_id=false;
                if(invoiceiteam[i].description=='' || invoiceiteam[i].description==null)  return this.adderrors.description=true;
                    else this.adderrors.description=false;
                if(invoiceiteam[i].quantity=='' || invoiceiteam[i].quantity==null)  return this.adderrors.quantity=true;
                    else this.adderrors.quantity=false;
                if(invoiceiteam[i].price=='' || invoiceiteam[i].price==null)  return this.adderrors.price=true;
                    else this.adderrors.price=false;
            }
            // this.$inertia.post('../../invoiceupdate/',{form: form,add:invoiceiteam});

                this.$inertia.post(this.route('invoice.update'),{form: form,add:invoiceiteam,itemdelete:this.itemdelete});
        },
        mail(){
            if(form.email==null)  return [this.errors.email=true,this.$refs.email.focus()];
               else this.errors.email=false;

            this.$inertia.get(this.route('invoice.mail'),{form: form});
        },
        paidstatus(form){
            if(form.paid_date==null)  return [this.errors.paid_date=true];
               else this.errors.paid_date=false;
            this.form.paid_status = true;
            Swal.fire({
            title: 'Do you want Change Paid Status?',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
            confirmButtonText: 'Yes,Paid',
                  ...
                  if (result.isConfirmed) {
                this.$inertia.post(this.route('invoicepaids'),{form: form})
                  }
            })
        },
