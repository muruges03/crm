            ...
            <div class="shadow-lg rounded-md">
                <h2 class="text-lg font-semibold flex px-4 py-2">
                    <inertia-link class="hover:text-black-600 mt-1 flex" :href="route('accounts.index')">
                        <img src="/assets/add-product.png" class="w-6 h-6 mr-3">Update Ledger
                    </inertia-link>
                </h2>
                <div class="px-4 py-4">
    ...
    data() {
        return {
            ...
            form: this.$inertia.form({
                id              : this.ledger.id,
                title           : this.ledger.title,
                account_type_id : this.ledger.account_type_id,
                notes           : null,
                type            : this.ledger.type,
                opening_balance : this.ledger.opening_balance,
            }),
        }
    },
    methods: {
        ...
        store() {
            // alert();
            this.form.put(this.route('ledger.update',this.ledger.id));
        },
    },
}
