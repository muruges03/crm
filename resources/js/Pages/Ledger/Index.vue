...
<template>
    ...
    <div class="lg:pl-64 flex flex-col bg-white divide-x divide-gray-200 border-2 shadow-md ">
        ...
        <div class="flex flex-col p-4">
            <div class="border-b border-gray-200 px-2 py-2 flex sm:items-center justify-between sm:px-4 lg:px-8">
                ...
                <div class=" sm:mt-0 sm:ml-4 " >
                    <jet-button type="button" >
                        <inertia-link v-bind:href="route('ledger.create')">Create</inertia-link>
                    </jet-button>
                </div>
            </div>
                    ...
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-3 py-3 text-md text-center  ">
                                    <inertia-link>Account Type</inertia-link>
                                </th>
                                <th class="px-3 py-3 text-md text-center  " ></th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-for="(ledgers, index) in ledger.data" :key="ledgers.id" v-bind:class="index % 2 === 0 ? 'bg-white-100 hover:bg-blue-100' : 'tr-bg-color hover:bg-blue-100'">
                                <td  class="px-3 py-3 text-sm text-center text-gray-900">
                                    <inertia-link  >{{ index+1 }}</inertia-link>
                                </td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ ledgers.title }}</inertia-link></td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ ledgers.type_name }}</inertia-link></td>
                                <td class="py-4 pl-3 pr-4 flex justify-center text-sm font-medium sm:pr-6">

                                    <inertia-link class="" v-bind:href="route('ledger.edit', ledgers.id)" >
                                    ...
                                    <a  class="btn btn-danger btn-sm" @click="destroy(ledgers.id)" >
            ...
            <pagination class="" :links="ledger.links" />
        </div>
    </div>
</template>
        ...
        destroy(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(this.route('ledger.destroy', id))
                }
            })
        },
        search() {
            this.$inertia.replace(this.route('ledger.index', {term: this.term}))
        },
