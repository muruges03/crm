            ...
            <div class="border-b border-gray-200 px-2 py-2 flex sm:items-center justify-between sm:px-4 lg:px-8">
                ...
                <div class=" sm:mt-0 sm:ml-4 " >
                    <jet-button type="button" >
                        <inertia-link v-bind:href="route('accounts.create')">Create</inertia-link>
                    </jet-button>
                </div>
            </div>
                ...
                <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody>
                            <tr v-for="(accounts, index) in account.data" :key="accounts.id" v-bind:class="index % 2 === 0 ? 'bg-white-100 hover:bg-blue-100' : 'tr-bg-color hover:bg-blue-100'">
                                ...
                                <td class="py-4 pl-3 pr-4 flex justify-center text-sm font-medium sm:pr-6">
                                    ...
                                    <inertia-link class="" v-bind:href="route('accounts.edit', accounts.id)" >
...
<!--            <div class="p-3 text-gray-500">Showing <b>{{firstItem}}</b> to <b>{{lastItem}}</b> of <b>{{total}}</b> Items </div>-->
            <pagination class="" :links="account.links" />
        </div>
    </div>
</template>
...
export default ({
    ...
    methods: {
        ...
        search() {
            this.$inertia.replace(this.route('accounts.index', {term: this.term}))
        },
        resetPagination(length){
            //console.log(length);
            this.$inertia.replace(this.route('account', {length: this.length}))
        },
    },
})
</script>
