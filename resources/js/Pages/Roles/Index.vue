        ...
        <div class="flex flex-col p-4">
            <div class="border-b border-gray-200 px-2 py-2 flex sm:items-center justify-between sm:px-2 lg:px-8">
                ...
                <div class=" sm:mt-0 sm:ml-4 ">
                    ...
                    <inertia-link :href="route('roles.create')">Create</inertia-link>
                    </button>
                </div>
            </div>
                ...
                <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody>
                                <tr v-for="(role, index) in roles.data" :key="role.id" v-bind:class="index % 2 === 0 ? 'bg-white-100 hover:bg-blue-100' : 'bg-gray-200 hover:bg-blue-100'">                              
                                    <td  class="px-3 py-3 whitespace-nowrap text-sm text-center text-gray-900">                       
                                        <inertia-link  v-bind:href="route('roles.edit', role.id)" >{{ ++index}}</inertia-link>
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap text-sm text-center text-gray-500"><inertia-link  v-bind:href="route('roles.edit', role.id)" >{{ role.name }}</inertia-link></td>
                                    <td class="px-3 py-3 whitespace-nowrap text-sm text-center text-gray-500"><inertia-link  v-bind:href="route('roles.edit', role.id)" >{{ role.guard_name }}</inertia-link></td>                                  
                                   <td class="px-3 py-3 flex text-sm justify-center text-gray-500">
                                        <inertia-link class="btn btn-danger btn-sm" v-bind:href="route('roles.destroy', role.id)" >
                                        ...
                                        <inertia-link class="" v-bind:href="route('roles.edit', role.id)">
    ...
    methods: {
         search() {
          ...
          resetPagination(length){
            //console.log(length);
            this.$inertia.replace(this.route('roles', {length: this.length}))
          },
    },
    })
</script>
