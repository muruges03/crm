        ...
        <div class="flex flex-col p-4">
            <div class="border-b border-gray-200 px-2 py-2 flex sm:items-center justify-between sm:px-4 lg:px-8">
                <h2 class=" text-2xl font-bold min-w-0">
                    Default Ledger Setting List
                </h2>
                <div class=" sm:mt-0 sm:ml-4 " >
                    <jet-button type="button" >
                        <inertia-link v-bind:href="route('defaultSetting.create')">Create</inertia-link>
                    </jet-button>
                </div>
            </div>
                    ...
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-3 py-3 text-md text-center  ">
                                    <inertia-link>Name</inertia-link>
                                </th>
                                <th scope="col" class="px-3 py-3 text-md text-center  ">
                                    <inertia-link>Ledger</inertia-link>
                                </th>
                                <th class="px-3 py-3 text-md text-center  " ></th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-for="(defaultSettings, index) in defaultsetting.data" :key="defaultSettings.id" v-bind:class="index % 2 === 0 ? 'bg-white-100 hover:bg-blue-100' : 'tr-bg-color hover:bg-blue-100'">
                                <td  class="px-3 py-3 text-sm text-center text-gray-900">
                                    <inertia-link  >{{ index+1 }}</inertia-link>
                                </td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ defaultSettings.type }}</inertia-link></td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ defaultSettings.name }}</inertia-link></td>
                                <td class="px-3 py-3 text-sm text-center text-gray-500"><inertia-link   >{{ defaultSettings.l_title }}</inertia-link></td>
                                <td class="py-4 pl-3 pr-4 flex justify-center text-sm font-medium sm:pr-6">

                                    <inertia-link class="" v-bind:href="route('defaultSetting.edit', defaultSettings.id)" >
                                    ...
                                    <a  class="btn btn-danger btn-sm" @click="destroy(defaultSettings.id)" >
            ...
            <pagination class="" :links="defaultsetting.links" />
        </div>
    </div>
</template>
        ...
        destroy(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$inertia.delete(this.route('defaultSetting.destroy', id))
                }
            })
        },
        search() {
            this.$inertia.replace(this.route('defaultSetting.index', {term: this.term}))
        },
        resetPagination(length){
            //console.log(length);
            this.$inertia.replace(this.route('defaultSetting', {length: this.length}))
        },
    },
})
</script>
